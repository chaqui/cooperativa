...
class BitacoraInteresService
{
    ...
    private function calcularInteresNormal($saldo, $interesMensual, $fechaInicio, $fechaFin)
    {
        ...
        $this->log("Calculando interés normal. Saldo: {$saldo}, Interés mensual: {$interesMensual}, Fecha inicio: {$fechaInicio->format('Y-m-d')}, Fecha fin: {$fechaFin->format('Y-m-d')}, Días transcurridos: {$diasTranscurridos}");
    ...
    public function calcularInteres(DateTime $fechaPago, $saldo, $interesMensual, DateTime $fechaUltimoPago, DateTime $fechaDeposito)
    {
        ...
        if ($fechaDeposito > $fechaLimite) {
            ...
            if($fechaUltimoPago> $fechaPago){ // Si el último pago es mayor a la fecha de pago, se cobra interés a la misma fecha del pago programado
